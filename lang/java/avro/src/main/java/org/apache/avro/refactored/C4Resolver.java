 ...
 */
// ===================================================================
// VARIANTE C4 - refactoring LLM con BB + riepilogo coverage gap + (se
// esistesse) una suite di test "MT" equivalente a TestUtf8MutationReinforced
// per Resolver.
// NOTA METODOLOGICA: per Resolver questa suite MT-specifica non esiste,
// perche' PIT (punto 4b) ha gia' misurato Test Strength 100% (21/21) sulla
// suite BB per questa classe - non ci sono mutanti sopravvissuti da uccidere
// con nuovi test di rinforzo. Di conseguenza il contesto disponibile per C4
// e' IDENTICO a quello di C3 per Resolver, e questa variante e' -
// deliberatamente - identica a C3Resolver.java (stesso codice, stesso
// insieme di refactoring applicati/evitati). Per Utf8 (che HA una suite MT
// dedicata) C3 e C4 potrebbero invece differire.
// ===================================================================
public class C4Resolver {
  /**
    ...
    static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (Field writeField : writeFields) {
        ...
        if (readField != null) {
          ...
          actions[i++] = C4Resolver.resolve(writeField.schema(), readField.schema(), data, seen);
        } else {
  ...
  public static class WriterUnion extends Action {
    ...
    public static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (int i = 0; i < writeTypeLength; i++) {
        actions[i] = C4Resolver.resolve(writeTypes.get(i), (unionEquivalent ? readTypes.get(i) : readSchema), data,
            seen);
      }
  ...
  public static class ReaderUnion extends Action {
    ...
    public static Action resolve(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      if (0 <= i) {
        return new ReaderUnion(w, r, d, i, C4Resolver.resolve(w, r.getTypes().get(i), d, seen));
      }
