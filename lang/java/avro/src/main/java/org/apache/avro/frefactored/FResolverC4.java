...
import org.apache.avro.FResolverC4.ErrorAction.ErrorType;

public class FResolverC4 {

  // Prevent instantiation: this is a utility holder for static resolution logic.
  private FResolverC4() {
  }
  ...
  public static class Promote extends Action {
    ...
    public static boolean isValid(Schema w, Schema r) {
      ...
      Schema.Type readerType = r.getType();
      Schema.Type writerType = w.getType();

      if (readerType == Schema.Type.LONG) {
        return writerType == Schema.Type.INT;
      }

      if (readerType == Schema.Type.FLOAT) {
        return writerType == Schema.Type.INT || writerType == Schema.Type.LONG;
      }

      if (readerType == Schema.Type.DOUBLE) {
        return writerType == Schema.Type.INT || writerType == Schema.Type.LONG || writerType == Schema.Type.FLOAT;
      }

      if (readerType == Schema.Type.BYTES || readerType == Schema.Type.STRING) {
        return writerType == Schema.Type.STRING || writerType == Schema.Type.BYTES;
      }
    ...
    static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (Field writeField : writeFields) {
        ...
        if (readField != null) {
          ...
          actions[i++] = FResolverC4.resolve(writeField.schema(), readField.schema(), data, seen);
        } else {
  ...
  public static class WriterUnion extends Action {
    ...
    public static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (int i = 0; i < writeTypeLength; i++) {
        actions[i] = FResolverC4.resolve(writeTypes.get(i), (unionEquivalent ? readTypes.get(i) : readSchema), data,
            seen);
      }
  ...
  public static class ReaderUnion extends Action {
    ...
    public static Action resolve(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      if (0 <= i) {
        return new ReaderUnion(w, r, d, i, FResolverC4.resolve(w, r.getTypes().get(i), d, seen));
      }
    ...
    private static int firstMatchingBranch(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      int exactMatchIndex = findExactOrStructureMatchIndex(w, r, d, seen, vt);
      if (exactMatchIndex >= 0) {
        return exactMatchIndex;
      }

      return findPromotionMatchIndex(w, r, vt);
    }

    private static int findExactOrStructureMatchIndex(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen,
        Schema.Type vt) {
      int j = 0;
    ...
    private static int findPromotionMatchIndex(Schema w, Schema r, Schema.Type vt) {
      int j = 0;
      for (Schema b : r.getTypes()) {
