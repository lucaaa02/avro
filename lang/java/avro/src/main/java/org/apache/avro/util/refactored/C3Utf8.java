...
// ===================================================================
// VARIANTE C3 - refactoring LLM con BB + riepilogo dei gap di coverage
// JaCoCo (setByteLength, equals boundary, hashCode segnalati come aree a
// rischio/scoperte).
// Rispetto a C2: ricevendo l'indicazione esplicita che equals() e
// hashCode() sono aree "a rischio" (gap di coverage), il refactor
// DIVENTA CONSERVATIVO su entrambi i metodi e li lascia ESATTAMENTE
// identici al codice originale (sia il merge di equals() sia il bug di
// hashCode() introdotti in C1/C2 sono annullati qui). Il refactor si
// sposta invece su un'area esplicitamente NON segnalata come a rischio:
// compareSequences(), dove la sottrazione manuale "a - b" tra char viene
// sostituita con Character.compare(a, b) (comportamento identico, per
// costruzione, dato che Character.compare(char,char) e' definito come
// "a - b").
// ===================================================================
...
/**
 * A C3Utf8 string. Unlike {@link String}, instances are mutable. This is more
 * efficient than {@link String} when reading or writing a sequence of values,
 * as a single instance may be reused.
 */
public class C3Utf8 implements Comparable<C3Utf8>, CharSequence, Externalizable {
  ...
  C3Utf8(String string, int length) {
    this(string);
    this.length = length;
  }
   ...
   * same as the current length, the cached contents of this C3Utf8 object will be
   * wiped out. After calling this method, no assumptions should be made about the
   * internal state (e.g., contents, hashcode, equality, etc.) of this C3Utf8
   ...
   * @see org.apache.avro.io.BinaryDecoder#readString(C3Utf8)
   */
  public C3Utf8 setByteLength(int newLength) {
  ...
  public C3Utf8 set(String string) {
    byte[] bytes = getBytesFor(string);
    int length = bytes.length;
    SystemLimitException.checkMaxStringLength(length);
    this.bytes = bytes;
  ...
  public C3Utf8 set(C3Utf8 other) {
  ...
  public boolean equals(Object o) {
    if (o == this)
      return true;
    if (!(o instanceof C3Utf8))
      return false;
    C3Utf8 that = (C3Utf8) o;
  ...
  @Override
  public int compareTo(C3Utf8 that) {
