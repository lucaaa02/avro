...
import org.apache.avro.C3Resolver.ErrorAction.ErrorType;
 ...
 */
// ===================================================================
// VARIANTE C3 - refactoring LLM con BB allegato + riepilogo testuale dei
// gap di coverage JaCoCo (righe/branch mai eseguiti dalla sola suite BB).
// Conoscendo ESATTAMENTE quali parti della classe non hanno una rete di
// sicurezza nei test, questa variante e' deliberatamente PIU' CONSERVATIVA
// di C1/C2:
//  - ErrorAction.error: non rinominato (come C2).
//  - Promote.isValid: refactoring APPLICATO (il grosso dei casi e' coperto
//    da BB - t09..t18).
//  - ErrorAction.toString/MISSING_REQUIRED_FIELD: NON TOCCATO. Il gap di
//    coverage indica che l'intero metodo toString() non e' mai chiamato
//    dalla suite BB (i test controllano il campo error/errorType
//    direttamente, mai .toString()). Notare che il codice originale, in
//    presenza di piu' campi mancanti, mantiene il nome dell'ULTIMO campo
//    trovato (nessun "break" nel loop) - una riscrittura "ovvia" con
//    stream+findFirst avrebbe silenziosamente invertito questa semantica
//    (primo anziche' ultimo match), un bug che la suite BB non avrebbe
//    rilevato. Lasciato invariato per questo motivo.
//  - RecordAdjust.noReorder(): NON TOCCATO, il gap di coverage mostra che
//    questo metodo pubblico non e' MAI invocato dalla suite BB.
//  - ReaderUnion.firstMatchingBranch/scan di promozione numerica: NON
//    TOCCATO, il gap di coverage mostra che questa porzione (matching
//    strutturale su RECORD/ENUM/FIXED e scan di promozione numerica) non
//    e' mai esercitata dalla suite BB (t30 la aggira trovando un match
//    esatto immediato).
// ===================================================================
public class C3Resolver {
  /**
    ...
    static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (Field writeField : writeFields) {
        ...
        if (readField != null) {
          ...
          actions[i++] = C3Resolver.resolve(writeField.schema(), readField.schema(), data, seen);
        } else {
  ...
  public static class WriterUnion extends Action {
    ...
    public static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (int i = 0; i < writeTypeLength; i++) {
        actions[i] = C3Resolver.resolve(writeTypes.get(i), (unionEquivalent ? readTypes.get(i) : readSchema), data,
            seen);
      }
  ...
  public static class ReaderUnion extends Action {
    ...
    public static Action resolve(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      if (0 <= i) {
        return new ReaderUnion(w, r, d, i, C3Resolver.resolve(w, r.getTypes().get(i), d, seen));
      }
