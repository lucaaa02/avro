...
// ===================================================================
// VARIANTE C2 - refactoring LLM con la sola suite BB allegata al prompt
// (TestUtf8CategoryPartition.java).
// Rispetto a C1: il rename di set(String)/set(Utf8) e' stato ANNULLATO,
// perche' il refactor si accorge (dalla firma dei metodi non referenziata
// nei test forniti, unita' a una scelta piu' cauta quando un contratto
// pubblico e' osservato da un test esistente) che alterare nomi di metodi
// pubblici e' rischioso senza sapere se sono usati altrove - infatti lo
// sono, da LLM/ES, che con C1 non compilavano piu'.
// Il refactor di equals() (sicuro, nessun cambio di comportamento) resta.
// Il BUG LATENTE di hashCode() introdotto in C1 (guardia "bytes.length ==
// length" rimossa) resta anch'esso INVARIATO: la suite BB fornita come
// contesto non esercita mai lo scenario "setByteLength in riduzione poi
// hashCode()", quindi nulla nel contesto disponibile segnala il rischio.
// ===================================================================
...
/**
 * A C2Utf8 string. Unlike {@link String}, instances are mutable. This is more
 * efficient than {@link String} when reading or writing a sequence of values,
 * as a single instance may be reused.
 */
public class C2Utf8 implements Comparable<C2Utf8>, CharSequence, Externalizable {
  ...
  C2Utf8(String string, int length) {
    this(string);
    this.length = length;
  }
   ...
   * same as the current length, the cached contents of this C2Utf8 object will be
   * wiped out. After calling this method, no assumptions should be made about the
   * internal state (e.g., contents, hashcode, equality, etc.) of this C2Utf8
   ...
   * @see org.apache.avro.io.BinaryDecoder#readString(C2Utf8)
   */
  public C2Utf8 setByteLength(int newLength) {
  ...
  public C2Utf8 set(String string) {
    byte[] bytes = getBytesFor(string);
    int length = bytes.length;
    SystemLimitException.checkMaxStringLength(length);
    this.bytes = bytes;
  ...
  public C2Utf8 set(C2Utf8 other) {
  ...
  public boolean equals(Object o) {
    if (o == this)
      return true;
    if (!(o instanceof C2Utf8))
      return false;
    C2Utf8 that = (C2Utf8) o;
  ...
  @Override
  public int compareTo(C2Utf8 that) {
