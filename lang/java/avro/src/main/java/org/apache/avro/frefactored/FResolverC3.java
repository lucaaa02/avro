...
import org.apache.avro.FResolverC3.ErrorAction.ErrorType;

public class FResolverC3 {
  ...
  private static Action resolve(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
    ...
    if (wType == rType) {
      return resolveSameType(w, r, d, seen, wType);
    }
  ...
  private static Action resolveSameType(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen,
      Schema.Type wType) {
    switch (wType) {
    case NULL:
    case BOOLEAN:
    ...
    static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (Field writeField : writeFields) {
        ...
        if (readField != null) {
          ...
          actions[i++] = FResolverC3.resolve(writeField.schema(), readField.schema(), data, seen);
        } else {
  ...
  public static class WriterUnion extends Action {
    ...
    public static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (int i = 0; i < writeTypeLength; i++) {
        actions[i] = FResolverC3.resolve(writeTypes.get(i), (unionEquivalent ? readTypes.get(i) : readSchema), data,
            seen);
      }
  ...
  public static class ReaderUnion extends Action {
    ...
    public static Action resolve(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      if (0 <= i) {
        return new ReaderUnion(w, r, d, i, FResolverC3.resolve(w, r.getTypes().get(i), d, seen));
      }
    ...
    private static int firstMatchingBranch(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      int structureMatch = findExactOrStructureMatch(w, r, d, seen, vt);
      if (structureMatch >= 0) {
        return structureMatch;
      }

      return findPromotionMatch(vt, r);
    }

    private static int findExactOrStructureMatch(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen,
        Schema.Type vt) {
      int j = 0;
    ...
    private static int findPromotionMatch(Schema.Type vt, Schema r) {
      int j = 0;
      for (Schema b : r.getTypes()) {
        if (isNumericOrStringPromotion(vt, b.getType())) {
          return j;
        }
        j++;
      }
    ...
    private static boolean isNumericOrStringPromotion(Schema.Type vt, Schema.Type bt) {
      if (vt == Schema.Type.INT) {
        return bt == Schema.Type.LONG || bt == Schema.Type.DOUBLE || bt == Schema.Type.FLOAT;
      }
      if (vt == Schema.Type.LONG) {
        return bt == Schema.Type.DOUBLE || bt == Schema.Type.FLOAT;
      }
      if (vt == Schema.Type.FLOAT) {
        return bt == Schema.Type.DOUBLE;
      }
      if (vt == Schema.Type.STRING) {
        return bt == Schema.Type.BYTES;
      }
      if (vt == Schema.Type.BYTES) {
        return bt == Schema.Type.STRING;
      }
      return false;
    }
