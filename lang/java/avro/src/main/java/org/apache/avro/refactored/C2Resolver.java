...
import org.apache.avro.C2Resolver.ErrorAction.ErrorType;
 ...
 */
// ===================================================================
// VARIANTE C2 - refactoring LLM con TestResolverCategoryPartition.java
// (BB) allegato al prompt.
// Differenza rispetto a C1: il campo pubblico ErrorAction.error NON viene
// rinominato, perche' il file di test allegato lo referenzia per nome
// (assertEquals(..., ((Resolver.ErrorAction) a).error)) - rinominarlo
// avrebbe rotto la compilazione dei test.
// Gli altri refactoring (Promote, toString, noReorder, firstMatchingBranch)
// sono applicati come in C1: il file di test non dice nulla su QUALI righe
// siano effettivamente coperte, quindi non c'e' motivo per evitarli.
// ===================================================================
public class C2Resolver {
  /**
   * Returns a {@link Action} tree for resolving the writer schema <tt>writer</tt>
   * and the reader schema <tt>reader</tt>.
   *
  ...
  public static class ErrorAction extends Action {
    ...
    public String toString() {
      ...
      case MISSING_REQUIRED_FIELD: {
        String fname = reader.getFields().stream()
            .filter(rf -> writer.getField(rf.name()) == null && rf.defaultValue() == null).map(Field::name).findFirst()
            .orElse("<oops>");
  ...
  public static class Promote extends Action {
     ...
     * Should
     */
    private static final Map<Schema.Type, java.util.Set<Schema.Type>> VALID_PROMOTIONS = buildValidPromotions();

    private static Map<Schema.Type, java.util.Set<Schema.Type>> buildValidPromotions() {
      Map<Schema.Type, java.util.Set<Schema.Type>> table = new java.util.EnumMap<>(Schema.Type.class);
      table.put(Schema.Type.LONG, java.util.EnumSet.of(Schema.Type.INT));
      table.put(Schema.Type.FLOAT, java.util.EnumSet.of(Schema.Type.INT, Schema.Type.LONG));
      table.put(Schema.Type.DOUBLE, java.util.EnumSet.of(Schema.Type.INT, Schema.Type.LONG, Schema.Type.FLOAT));
      table.put(Schema.Type.STRING, java.util.EnumSet.of(Schema.Type.STRING, Schema.Type.BYTES));
      table.put(Schema.Type.BYTES, java.util.EnumSet.of(Schema.Type.STRING, Schema.Type.BYTES));
      return table;
    }
    ...
    public static boolean isValid(Schema w, Schema r) {
      ...
      java.util.Set<Schema.Type> validWriterTypes = VALID_PROMOTIONS.get(r.getType());
      return validWriterTypes != null && validWriterTypes.contains(w.getType());
    }
  }
  ...
  public static class RecordAdjust extends Action {
    ...
    public boolean noReorder() {
      return java.util.stream.IntStream.range(0, readerOrder.length).allMatch(i -> i == readerOrder[i].pos());
    }
    ...
    static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (Field writeField : writeFields) {
        ...
        if (readField != null) {
          ...
          actions[i++] = C2Resolver.resolve(writeField.schema(), readField.schema(), data, seen);
        } else {
  ...
  public static class WriterUnion extends Action {
    ...
    public static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (int i = 0; i < writeTypeLength; i++) {
        actions[i] = C2Resolver.resolve(writeTypes.get(i), (unionEquivalent ? readTypes.get(i) : readSchema), data,
            seen);
      }
  ...
  public static class ReaderUnion extends Action {
    ...
    public static Action resolve(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      if (0 <= i) {
        return new ReaderUnion(w, r, d, i, C2Resolver.resolve(w, r.getTypes().get(i), d, seen));
      }
    ...
    private static final Map<Schema.Type, java.util.Set<Schema.Type>> NUMERIC_PROMOTION_TARGETS = buildPromotionTargets();

    private static Map<Schema.Type, java.util.Set<Schema.Type>> buildPromotionTargets() {
      Map<Schema.Type, java.util.Set<Schema.Type>> table = new java.util.EnumMap<>(Schema.Type.class);
      table.put(Schema.Type.INT, java.util.EnumSet.of(Schema.Type.LONG, Schema.Type.DOUBLE, Schema.Type.FLOAT));
      table.put(Schema.Type.LONG, java.util.EnumSet.of(Schema.Type.DOUBLE, Schema.Type.FLOAT));
      table.put(Schema.Type.FLOAT, java.util.EnumSet.of(Schema.Type.DOUBLE));
      table.put(Schema.Type.STRING, java.util.EnumSet.of(Schema.Type.BYTES));
      table.put(Schema.Type.BYTES, java.util.EnumSet.of(Schema.Type.STRING));
      return table;
    }
    ...
    private static int firstMatchingBranch(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      java.util.Set<Schema.Type> promotionTargets = NUMERIC_PROMOTION_TARGETS.getOrDefault(vt,
          java.util.Collections.emptySet());
      j = 0;
      for (Schema b : r.getTypes()) {
        if (promotionTargets.contains(b.getType())) {
          return j;
        }
        j++;
      }
