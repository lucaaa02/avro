...
import org.apache.avro.C1Resolver.ErrorAction.ErrorType;
...
//  1) ErrorAction.error rinominato in errorType (nome piu' convenzionale,
//     coerente con la doc ufficiale Avro) - CAMBIA la firma pubblica.
//  2) Promote.isValid: tabella statica al posto degli switch annidati.
//  3) ErrorAction.toString/MISSING_REQUIRED_FIELD: loop -> stream.
//  4) RecordAdjust.noReorder: loop -> IntStream.allMatch.
//  5) ReaderUnion.firstMatchingBranch: scan di promozione numerica ->
//     tabella statica al posto degli switch annidati.
// ===================================================================
public class C1Resolver {
  /**
    ...
    static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (Field writeField : writeFields) {
        ...
        if (readField != null) {
          ...
          actions[i++] = C1Resolver.resolve(writeField.schema(), readField.schema(), data, seen);
        } else {
  ...
  public static class WriterUnion extends Action {
    ...
    public static Action resolve(Schema writeSchema, Schema readSchema, GenericData data, Map<SeenPair, Action> seen) {
      ...
      for (int i = 0; i < writeTypeLength; i++) {
        actions[i] = C1Resolver.resolve(writeTypes.get(i), (unionEquivalent ? readTypes.get(i) : readSchema), data,
            seen);
      }
  ...
  public static class ReaderUnion extends Action {
    ...
    public static Action resolve(Schema w, Schema r, GenericData d, Map<SeenPair, Action> seen) {
      ...
      if (0 <= i) {
        return new ReaderUnion(w, r, d, i, C1Resolver.resolve(w, r.getTypes().get(i), d, seen));
      }
